...
FPixelShaderRHIRef FD3D12DynamicRHI::RHICreatePixelShader(const TArray<uint8>& Code, int NvShaderExtnSlot)// CATACLYSM more atomic float ops in shaders
{
	check(NvShaderExtnSlot < 0);//CATACLYSM more atomic float ops in shaders
	FShaderCodeReader ShaderCode(Code);
...
FComputeShaderRHIRef FD3D12DynamicRHI::RHICreateComputeShader(const TArray<uint8>& Code, int NvShaderExtnSlot)//CATACLYSM more atomic float ops in shaders
{
	check(NvShaderExtnSlot < 0);//CATACLYSM more atomic float ops in shaders
	FShaderCodeReader ShaderCode(Code);

...
#define ContextRedirect(Call) PhysicalContexts[CurrentDeviceIndex]->##Call
	...
	FORCEINLINE virtual void RHISetRenderTargets(uint32 NumSimultaneousRenderTargets, const FRHIRenderTargetView* NewRenderTargets, const FRHIDepthRenderTargetView* NewDepthStencilTarget, uint32 NumUAVs, const FUnorderedAccessViewRHIParamRef* UAVs, int initUAVCount) final override// CATACLYSM more atomic float ops in shaders
	{
		ContextRedirect(RHISetRenderTargets(NumSimultaneousRenderTargets, NewRenderTargets, NewDepthStencilTarget, NumUAVs, UAVs, initUAVCount));// CATACLYSM more atomic float ops in shaders
	}

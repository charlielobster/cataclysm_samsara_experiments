...
public:
	...
	virtual void RHIUpdateTileMappings(FTextureRHIParamRef Texture, uint32 NumTiledResourceRegions, const TArray<FTiledResourceCoordinate>& StartCoordinates, const TArray<FTileRegionSize>& Sizes,
									   FTilePoolRHIParamRef TilePool, uint32 NumRanges, const TArray<uint32>& Flags, const TArray<uint32>& StartOffsets, const TArray<uint32>& Counts) final override
	{
		// TODO
	}
	...
	virtual void RHISetRenderTargets(uint32 NumSimultaneousRenderTargets, const FRHIRenderTargetView* NewRenderTargets, const FRHIDepthRenderTargetView* NewDepthStencilTarget, uint32 NumUAVs, const FUnorderedAccessViewRHIParamRef* UAVs, int initUAVCount = -1) final override; // CATACLYSM Extend RHI

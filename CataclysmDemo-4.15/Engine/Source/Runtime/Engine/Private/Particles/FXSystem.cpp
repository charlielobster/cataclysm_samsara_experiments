...
namespace FXConsoleVariables
{
	...
	int32 MaxLiquidParticleTilePreAllocation = 10000;// CATACLYSM
	...
	// BEGIN CATACLYSM
	FAutoConsoleVariableRef CVarMaxLiquidParticleTilePreAllocation(
		TEXT("FX.MaxLiquidParticleTilePreAllocation"),
		MaxLiquidParticleTilePreAllocation,
		TEXT("Maximum tile preallocation for Liquid particles."),
		ECVF_Cheat
		);
	// END CATACLYSM
...
#endif // #if WITH_EDITOR
{
	...
	{
		ParticleSimulationResources[Type] = nullptr;
	}
	// CATACLYSM End
	InitGPUSimulation();
}
...
void FFXSystem::PreRender(FRHICommandListImmediate& RHICmdList, const FGlobalDistanceFieldParameterData* GlobalDistanceFieldParameterData)
{
	...
	{
		// CATACLYSM Begin
		for (int32 i = 0; i < ELiquidParticleType::TYPE_MAX; ++i)
		{
			UpdateMultiGPUResources(RHICmdList, i);
		}
		// CATACLYSM End
		...
		if (FluidSimulation)
		{
			PrepareGPUSimulation(RHICmdList, ELiquidParticleType::Liquid);
			SimulateFluidGPUParticles(RHICmdList, GlobalDistanceFieldParameterData);
			FinalizeGPUSimulation(RHICmdList, ELiquidParticleType::Liquid);

			PrepareGPUSimulation(RHICmdList, ELiquidParticleType::Spray);
			SimulateSprayGPUParticles(RHICmdList, GlobalDistanceFieldParameterData);
			FinalizeGPUSimulation(RHICmdList, ELiquidParticleType::Spray);
			// Foam
			PrepareGPUSimulation(RHICmdList, ELiquidParticleType::Foam);
			SimulateFoamGPUParticles(RHICmdList, GlobalDistanceFieldParameterData);
			FinalizeGPUSimulation(RHICmdList, ELiquidParticleType::Foam);
		}
		// CATACLYSM End
...
// CATACLYSM Begin
void FFXSystem::Visualize(FPrimitiveDrawInterface* PDI)
{
	if (FluidConsoleVariables::ShowBricks && PDI)
	{
		if (FluidSimulation) FluidSimulation->Visualize(PDI);
	}
}

void FFXSystem::RegisterFluidSimulation(FFluidSimulation *FluidSimulationLocal)
{
	ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(
		FRegisterFluidCommand,
		FFXSystem*, FXSystem, this,
		FFluidSimulation*, Instance, FluidSimulationLocal,
	{
		// if we have an old fluid system in here, delete it before we register a new one.
		if (FXSystem->FluidSimulation)
		{
			FXSystem->FluidSimulation->Destroy();
			FXSystem->FluidSimulation = nullptr;
		}
		FXSystem->FluidSimulation = Instance;
		FXSystem->FluidSimulation->InitResources(&FXSystem->FluidRegions);
	});
}

void FFXSystem::UnregisterFluidSimulation()
{
	ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER(
		FUnregisterFluidCommand,
		FFXSystem*, FXSystem, this,
	{
		if ( FXSystem->FluidSimulation )
		{
			FXSystem->FluidSimulation->Destroy();
			FXSystem->FluidSimulation = NULL;
		}
	});
}

void FFXSystem::AddFluidRegion(UShapeComponent* Shape, int32 Type, const FVector4& VelocityAndWeight, uint32 RegionFlags)
{
	check( Shape->FXSystem == this );

	FFluidRegion* FluidRegion = new FFluidRegion();

	// This is not suitable to use component at the rendering thread.
	FluidRegion->ShapeComponent = Shape;
	
	FluidRegion->Type = Type;
	FluidRegion->VelocityAndWeight = VelocityAndWeight;
	FluidRegion->Bounds = Shape->Bounds.GetBox();
	FluidRegion->VolumeToWorld = Shape->ComponentToWorld.ToMatrixNoScale();
	FluidRegion->RegionFlags = RegionFlags;

	UBoxComponent* BoxCollisionComponent = CastChecked<UBoxComponent>(Shape);
	if (BoxCollisionComponent)
	{
		FluidRegion->VolumeSize = BoxCollisionComponent->GetScaledBoxExtent() * 2.0f;
	}

	//TODO sphere? capsule?

	Shape->FluidRegion = FluidRegion;

	ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(
		FAddFluidRegionCommand,
		FFXSystem*, FXSystem, this,
		FFluidRegion*, FluidRegion, FluidRegion,
	{
		FluidRegion->Index = FXSystem->FluidRegions.AddUninitialized().Index;
		FXSystem->FluidRegions[ FluidRegion->Index ] = FluidRegion;
	});
}

void FFXSystem::RemoveFluidRegion(UShapeComponent* Shape)
{
	check( Shape->FXSystem == this );

	FFluidRegion* FluidRegion = Shape->FluidRegion;
	Shape->FluidRegion->ShapeComponent = NULL;
	Shape->FluidRegion = NULL;

	if ( FluidRegion )
	{
		ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(
			FRemoveFluidRegionCommand,
			FFXSystem*, FXSystem, this,
			FFluidRegion*, FluidRegion, FluidRegion,
		{
			if ( FluidRegion->Index != INDEX_NONE )
			{
				FXSystem->FluidRegions.RemoveAt( FluidRegion->Index );
				delete FluidRegion;
			}
		});
	}
...
void FFXSystem::UpdateFluidRegion(UShapeComponent* Shape)
{
	check( Shape->FXSystem == this );
	...
	if ( FluidRegion )
	{
		struct FUpdateFluidRegionParams
		{
			FBox Bounds;
			FMatrix VolumeToWorld;
		};

		FUpdateFluidRegionParams UpdateParams;
		UpdateParams.Bounds = Shape->Bounds.GetBox();
		UpdateParams.VolumeToWorld = Shape->ComponentToWorld.ToMatrixNoScale();

		ENQUEUE_UNIQUE_RENDER_COMMAND_THREEPARAMETER(
			FUpdateFluidRegionCommand,
			FFXSystem*, FXSystem, this,
			FFluidRegion*, FluidRegion, FluidRegion,
			FUpdateFluidRegionParams, UpdateParams, UpdateParams,
		{
			FluidRegion->Bounds = UpdateParams.Bounds;
			FluidRegion->VolumeToWorld = UpdateParams.VolumeToWorld;
		});
	}
}

bool FFXSystem::GetFluidRenderingParams(struct FFluidRenderingParams& OutParams) const
{
	if (FluidSimulation)
	{
		FluidSimulation->GetFluidRenderingParams(OutParams);
		return true;
	}
	else
	{

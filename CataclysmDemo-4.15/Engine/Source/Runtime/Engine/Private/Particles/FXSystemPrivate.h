...
public:
	...
	// CATACLYSM Begin
	virtual void Visualize(FPrimitiveDrawInterface* PDI) override;
	virtual void RegisterFluidSimulation(FFluidSimulation* FluidSimulation) override;
	virtual void UnregisterFluidSimulation() override;
	virtual FFluidSimulation *GetFluidSimulation() const override { return FluidSimulation; }
	virtual bool GetFluidRenderingParams(struct FFluidRenderingParams& OutParams) const override;
	virtual void AddFluidRegion(UShapeComponent* Shape, int32 Type, const FVector4& VelocityAndWeight, uint32 RegionFlags) override;
	virtual void RemoveFluidRegion(UShapeComponent* Shape) override;
	virtual void UpdateFluidRegion(UShapeComponent* Shape) override;
	// CATACLYSM End
	 ...
	 */
	// BEGIN CATACLYSM
	FParticleSimulationResources* GetParticleSimulationResources(int ParticleType)
	{
		return ParticleSimulationResources[ParticleType];
	}
	// END CATACLYSM
	...
	// BEGIN CATACLYSM
	void PrepareGPUSimulation(FRHICommandListImmediate& RHICmdList, int32 ParticleType = ELiquidParticleType::None);
	void FinalizeGPUSimulation(FRHICommandListImmediate& RHICmdList, int32 ParticleType = ELiquidParticleType::None);
	// END CATACLYSM
...
private:
	...
	*/
	void UpdateMultiGPUResources(FRHICommandListImmediate& RHICmdList, int32 ParticleType); // CATACLYSM
	...
	// CATACLYSM Begin
	void ClearParticlSimulationStateTexuresIfNecessary(FRHICommandListImmediate& RHICmdList, int32 ParticleType);
	void SimulateFluidGPUParticles(FRHICommandListImmediate& RHICmdList, const FGlobalDistanceFieldParameterData* GlobalDistanceFieldParameterData);
	// Foam
	void SimulateFoamGPUParticles(FRHICommandListImmediate& RHICmdList, const FGlobalDistanceFieldParameterData* GlobalDistanceFieldParameterData);
	// Spray
	void SimulateSprayGPUParticles(FRHICommandListImmediate& RHICmdList, const FGlobalDistanceFieldParameterData* GlobalDistanceFieldParameterData);
	// CATACLYSM End
...
private:
	...
	FParticleSimulationResources* ParticleSimulationResources[ELiquidParticleType::TYPE_MAX];// CATACLYSM
	...
	TSparseArray<class FFluidRegion*> FluidRegions;
	float LastDeltaSeconds;
	/** Previous frame new particles for multi-gpu simulation*/
	TArray<FNewParticle> LastFrameNewParticles[ELiquidParticleType::TYPE_MAX];
	// CATACLYSM End

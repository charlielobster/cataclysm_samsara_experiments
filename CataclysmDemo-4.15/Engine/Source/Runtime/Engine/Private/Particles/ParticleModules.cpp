...
void UParticleModuleAcceleration::CompileModule( FParticleEmitterBuildInfo& EmitterInfo )
{
	...
	EmitterInfo.SpawnModules.Add(this);  // CATACLYSM Acceleration gpu modules
}
...
#if 0 // CATACLYSM Acceleration gpu modules
...
#endif // CATACLYSM Acceleration gpu modules
...
void UParticleModuleTypeDataGpu::Build( FParticleEmitterBuildInfo& EmitterBuildInfo )
{
	...
	if ( InterpMethod != PSUVIM_None )
	{
		...
		if ( InterpMethod == PSUVIM_Random || InterpMethod == PSUVIM_Random_Blend )
		{
			if (EmitterBuildInfo.RequiredModule->RandomImageChanges == 0)
			{
				EmitterBuildInfo.RequiredModule->RandomImageTime	= 1.0f;
			}
			else
			{
				EmitterBuildInfo.RequiredModule->RandomImageTime	= 0.99f / (EmitterBuildInfo.RequiredModule->RandomImageChanges + 1);
			}

			ResourceData.RandomImageTime = EmitterBuildInfo.RequiredModule->RandomImageTime;
		}
		else
		{
			ResourceData.RandomImageTime = 0.0f;
		}
	}
	else
	{
	...
	EmitterInfo.RandomSubImage = (InterpMethod == PSUVIM_Random || InterpMethod == PSUVIM_Random_Blend);
	...
	// BEGIN CATACLYSM
	EmitterInfo.LiquidType = (ELiquidParticleType::Type)EmitterBuildInfo.LiquidType;
	EmitterInfo.EventType = EmitterBuildInfo.EventType;
	EmitterInfo.SpraySpawnPercentage = EmitterBuildInfo.SpraySpawnPercentage;
	// END CATACLYSM
#endif

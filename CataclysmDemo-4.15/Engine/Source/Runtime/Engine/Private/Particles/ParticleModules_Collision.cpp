...
UParticleModuleCollisionGPU::UParticleModuleCollisionGPU(const FObjectInitializer& ObjectInitializer)
	...
	// BEGIN CATACLYSM
	, LiquidType(ELiquidParticleType::Type::None)
	, bCollisionSprayEvent(false)
	, bEscapeSprayEvent(false)
	, bSplashSprayEvent(false)
	, SpraySpawnPercentage(1.0f)
	// END CATACLYSM
{
	bSpawnModule = false;
	bUpdateModule = false;
...
void UParticleModuleCollisionGPU::CompileModule(struct FParticleEmitterBuildInfo& EmitterInfo)
{
	...
	EmitterInfo.LiquidType = LiquidType; // CATACLYSM
	...
	// BEGIN CATACLYSM
	if(bCollisionSprayEvent) { EmitterInfo.EventType |= (1 << EFluidEventType::Event1); } 
	if(bEscapeSprayEvent) { EmitterInfo.EventType |= (1 << EFluidEventType::Event2); } 
	if(bSplashSprayEvent) { EmitterInfo.EventType |= (1 << EFluidEventType::Event3); } 
	EmitterInfo.SpraySpawnPercentage = SpraySpawnPercentage;
	// END CATACLYSM
}

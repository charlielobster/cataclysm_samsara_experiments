...
void FParticleScratchVertexBuffer::InitRHI()
{
	...
	// BEGIN CATACLYSM Spray
	if (GetFeatureLevel() >= ERHIFeatureLevel::SM5)
	{
		Flags = BUF_ShaderResource /*| BUF_ByteAddressBuffer*/ | BUF_UnorderedAccess;
		SprayVertexBufferRHI = RHICreateVertexBuffer(GParticleScratchVertexBufferSize, Flags, CreateInfo);
		SprayVertexBufferSRV = RHICreateShaderResourceView(VertexBufferRHI, /*Stride=*/ sizeof(float), PF_R32_FLOAT);
		SprayVertexBufferUAV = RHICreateUnorderedAccessView(SprayVertexBufferRHI, PF_R32_FLOAT);
	}
	// END CATACLYSM Spray
}
...
// BEGIN CATACLYSM Spray
FParticleBufferParamRef FParticleScratchVertexBuffer::GetSprayBufferParam()
{
	return SprayVertexBufferRHI;
}

FParticleShaderParamRef FParticleScratchVertexBuffer::GetSprayShaderParam()
{
	return SprayVertexBufferSRV;
}

FUnorderedAccessViewRHIParamRef FParticleScratchVertexBuffer::GetSprayUAVParam()
{
	return SprayVertexBufferUAV;
}
// END CATACLYSM Spray
...
void FParticleScratchVertexBuffer::ReleaseRHI()
{
	...
	// BEGIN CATACLYSM Spray
	SprayVertexBufferRHI.SafeRelease();
	SprayVertexBufferSRV.SafeRelease();
	SprayVertexBufferUAV.SafeRelease();
	// END CATACLYSM Spray

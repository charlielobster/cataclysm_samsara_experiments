...
{
	if (Template)// CATACLYSM This is called when template is null... ensuring it ensures a crash. ensure(Template))
	{
...
// BEGIN CATACLYSM
void UParticleSystemComponent::SetSpraySpawnPercentage(int32 EmitterIndex, float NewPercentage)
{
	ForceAsyncWorkCompletion(STALL);
	if ((EmitterIndex >= 0) && (EmitterIndex < EmitterInstances.Num()))
	{
		...
		if (EmitterInst)
		{
			EmitterInst->SetSpraySpawnPercentage(NewPercentage);
		}
	}
}
// END CATACLYSM

...
namespace FXConsoleVariables
{
	...
	// BEGIN CATACLYSM
	/** Maximum tile preallocation for Liquid particles. */
	extern int32 MaxLiquidParticleTilePreAllocation;
	// END CATACLYSM
...
public:
	...
	// CATACLYSM Begin
	virtual void Visualize(FPrimitiveDrawInterface* PDI) = 0;
	virtual void RegisterFluidSimulation(class FFluidSimulation* FluidSimulation) = 0;
	virtual void UnregisterFluidSimulation() = 0;
	virtual class FFluidSimulation* GetFluidSimulation() const = 0;
	virtual bool GetFluidRenderingParams(struct FFluidRenderingParams& OutParams) const = 0;

	virtual void AddFluidRegion(class UShapeComponent* Shape, int32 Type, const FVector4& VelocityAndWeight, uint32 RegionFlags) = 0;
	virtual void RemoveFluidRegion(class UShapeComponent* Shape) = 0;
	virtual void UpdateFluidRegion(class UShapeComponent* Shape) = 0;
	// CATACLYSM End

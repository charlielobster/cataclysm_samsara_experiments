...
IMPLEMENT_TEMPORALAA_PIXELSHADER_TYPE(5, 0, TEXT("FluidTemporalAAPS")); // CATACLYSM
...
void FRCPassPostProcessTemporalAA::Process(FRenderingCompositePassContext& Context)
{
	...
	{
		// BEGIN CATACLYSM
		{	// Fluid feedback for tagged pixels
			// Draw to pixels where stencil == 2
			Context.RHICmdList.SetDepthStencilState(TStaticDepthStencilState<false, CF_Always, true, CF_Equal, SO_Keep, SO_Keep, SO_Keep>::GetRHI(), 2);

			TShaderMapRef< FPostProcessTonemapVS >			VertexShader(Context.GetShaderMap());
			TShaderMapRef< FPostProcessTemporalAAPS<5, 0> >	PixelShader(Context.GetShaderMap());
	...
	else
	{
		...
		{	// Responsive feedback for tagged pixels
			// Draw to pixels where stencil == 1
			Context.RHICmdList.SetDepthStencilState(TStaticDepthStencilState<
				false, CF_Always,
				true, CF_Equal, SO_Keep, SO_Keep, SO_Zero,
				false, CF_Always, SO_Keep, SO_Keep, SO_Keep,

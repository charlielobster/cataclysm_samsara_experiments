...
void FDeferredShadingSceneRenderer::RenderTranslucencyParallel(FRHICommandListImmediate& RHICmdList)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
	{
...

		{
			...
			//CATACLYSM Begin
			// TODO I just copied and pasted this without really knowing how parallel trans rendering works...
			// so this could very well do bad things... maybe it's better to set r.ParallelTranslucency 0 or have
			// someone who knows what they are doing check this.
			{
				SCOPED_DRAW_EVENT(RHICmdList, FluidRendering);
				GFluidRendering.RenderFluid(*this, RHICmdList);
			}
			//CATACLYSM End

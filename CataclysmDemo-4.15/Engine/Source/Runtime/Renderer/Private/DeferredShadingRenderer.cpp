...
static TAutoConsoleVariable<float> CVarHBAOExponent(
	TEXT("r.HBAO.Exponent"),
	3.0f,
	TEXT("HBAO+ power exponent"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarHBAORadius(
	TEXT("r.HBAO.Radius"),
	1.0f,
	TEXT("HBAO+ radius"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarHBAOBias(
	TEXT("r.HBAO.Bias"),
	0.1f,
	TEXT("HBAO+ bias"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarHBAONearAO(
	TEXT("r.HBAO.NearAO"),
	1.0f,
	TEXT("HBAO+ near AO"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarHBAOFarAO(
	TEXT("r.HBAO.FarAO"),
	1.0f,
	TEXT("HBAO+ far AO"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarHBAOForegroundViewDepth(
	TEXT("r.HBAO.ForegroundViewDepth"),
	-1.0f,
	TEXT("HBAO+ foreground view depth"),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarHBAOBackgroundViewDepth(
	TEXT("r.HBAO.BackgroundViewDepth"),
	-1.0f,
	TEXT("HBAO+ background view depth"),
	ECVF_RenderThreadSafe);
...
#if WITH_GFSDK_SSAO
			// NVCHANGE_BEGIN: Add HBAO+
			Views[ViewIndex].FinalPostProcessSettings.HBAOEnable = CVarHBAOEnable.GetValueOnRenderThread();
			// NVCHANGE_END: Add HBAO+
#endif
		...
		ViewFamily.EngineShowFlags.HBAO)
	{
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
		{
				...
				View.FinalPostProcessSettings.HBAOPowerExponent > 0.f)
			{
				...
				// Get HBAO parameters from CVar as well

				GFSDK_SSAO_Parameters Params;
				
				Params.Radius = CVarHBAORadius.GetValueOnRenderThread();
				Params.Bias = CVarHBAOBias.GetValueOnRenderThread();
				Params.PowerExponent = CVarHBAOExponent.GetValueOnRenderThread();
				Params.NearAO = CVarHBAONearAO.GetValueOnRenderThread();
				Params.FarAO = CVarHBAOFarAO.GetValueOnRenderThread();
				Params.ForegroundAO.Enable = CVarHBAOForegroundViewDepth.GetValueOnRenderThread() > 0.0f ? true : false;
				Params.ForegroundAO.ForegroundViewDepth = CVarHBAOForegroundViewDepth.GetValueOnRenderThread();
				Params.BackgroundAO.Enable = CVarHBAOBackgroundViewDepth.GetValueOnRenderThread() > 0.0f ? true : false;
				Params.BackgroundAO.BackgroundViewDepth = CVarHBAOBackgroundViewDepth.GetValueOnRenderThread();
				...
				Params.Blur.Radius = View.FinalPostProcessSettings.HBAOBlurRadius == AOBR_BlurRadius4 ? GFSDK_SSAO_BLUR_RADIUS_4 : GFSDK_SSAO_BLUR_RADIUS_2;

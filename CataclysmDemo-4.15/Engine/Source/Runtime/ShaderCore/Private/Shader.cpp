...
FShaderResource::FShaderResource()
	...
	// CATACLYSM Begin more atomic float ops in shaders
	, NvShaderExtnSlot(-1)
	// CATACLYSM End more atomic float ops in shaders
{
...
void FShaderResource::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		...
		FShaderCache::LogShader((EShaderPlatform)Target.Platform, (EShaderFrequency)Target.Frequency, OutputHash, Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
...
void FShaderResource::InitRHI()
{
	...
	else if(Target.Frequency == SF_Pixel)
	{
		if (ShaderCache)
		{
			PixelShader = ShaderCache->GetPixelShader((EShaderPlatform)Target.Platform, OutputHash, UncompressedCode, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
		}
		else
		{
			PixelShader = RHICreatePixelShader(UncompressedCode, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
	...
	else if(Target.Frequency == SF_Compute)
	{
		if (ShaderCache)
		{
			ComputeShader = ShaderCache->GetComputeShader((EShaderPlatform)Target.Platform, UncompressedCode, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
		}
		else
		{
			ComputeShader = RHICreateComputeShader(UncompressedCode, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
		}
...
bool FShader::SerializeBase(FArchive& Ar, bool bShadersInline)
{
	...
	if (bShadersInline)
	{
		...
		if (Ar.IsLoading())
		{
			...
			SerializedResource->NvShaderExtnSlot = GetNvShaderExtnSlot(); // CATACLYSM more atomic float ops in shaders
...
void ShaderMapAppendKeyString(EShaderPlatform Platform, FString& KeyString)
{
	...
	{
		static const auto CVar = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.SceneAlpha"));
		if (CVar && CVar->GetValueOnAnyThread() > 0)
		{
			KeyString += TEXT("_SA");
		}

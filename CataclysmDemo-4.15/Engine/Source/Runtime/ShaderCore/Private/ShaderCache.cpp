				...
				for ( auto Entry : Cache->CodeCache.Shaders )
				{
					...
					if ( bUsable )
					{
						if(!bUseAsyncShaderPrecompilation)
						{
							...
							Cache->InternalLogShader(Entry.Key.Platform, Entry.Key.Frequency, Entry.Key.SHAHash, Code, -1); // Cataclysm - TO CONFIRM // CATACLYSM more atomic float ops in shaders
						}
						else
						{
...
FPixelShaderRHIRef FShaderCache::GetPixelShader(EShaderPlatform Platform, FSHAHash Hash, TArray<uint8> const& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
{
	FShaderCacheKey Key;
	Key.Platform = Platform;
	...
	if(!IsValidRef(Shader))
	{
		FShaderPlatformCache& PlatformCache = Caches.PlatformCaches.FindOrAdd(Platform);
		PlatformCache.Shaders.Add(Key);
		Shader = RHICreatePixelShader(Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
		check(IsValidRef(Shader));
		Shader->SetHash(Hash);
		INC_DWORD_STAT(STATGROUP_NumShadersCached);
...
FComputeShaderRHIRef FShaderCache::GetComputeShader(EShaderPlatform Platform, TArray<uint8> const& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
{
	FShaderCacheKey Key;
	Key.Platform = Platform;
	...
	if(!IsValidRef(Shader))
	{
		FShaderPlatformCache& PlatformCache = Caches.PlatformCaches.FindOrAdd(Platform);
		PlatformCache.Shaders.Add(Key);
		Shader = RHICreateComputeShader(Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
		check(IsValidRef(Shader));
		INC_DWORD_STAT(STATGROUP_NumShadersCached);
...
void FShaderCache::InternalLogShader(EShaderPlatform Platform, EShaderFrequency Frequency, FSHAHash Hash, TArray<uint8> const& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
{
	...
	if ( bUsable )
	{
		...
		ENQUEUE_UNIQUE_RENDER_COMMAND_FOURPARAMETER(LogShader, FShaderCache*,ShaderCache,this, FShaderCacheKey,Key,Key, TArray<uint8>,Code,Code, int,NvShaderExtnSlot,NvShaderExtnSlot, {
			...
			{
				ShaderCache->SubmitShader(Key, Code, NvShaderExtnSlot); /* CATACLYSM more atomic float ops in shaders*/
			}
		});
	}
}
	...
	if(FrameNum != GFrameNumberRenderThread || OverridePrecompileTime != 0 || OverridePredrawBatchTime != 0)
	{
		...
		if ( bUseShaderBinaryCache && bUseAsyncShaderPrecompilation && ShadersToPrecompile.Num() > 0 )
		{
			...
			for( uint32 Index = 0; (GetTargetPrecompileFrameTime() == -1 || NumCompiled < NumShadersToCompile) && Index < (uint32)ShadersToPrecompile.Num(); ++Index )
			{
				...
				SubmitShader(Key, Code, -1); // Cataclysm - TO CONFIRM // CATACLYSM more atomic float ops in shaders
...
void FShaderCache::SubmitShader(FShaderCacheKey const& Key, TArray<uint8> const& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
{
	FShaderPlatformCache& PlatformCache = Caches.PlatformCaches.FindOrAdd(Key.Platform);
	switch (Key.Frequency)
	{
		...
		case SF_Pixel:
			...
			{
				FPixelShaderRHIRef Shader = RHICreatePixelShader(Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
				if(IsValidRef(Shader))
				{
					Shader->SetHash(Key.SHAHash);
					INC_DWORD_STAT(STATGROUP_NumShadersCached);
		...
		case SF_Compute:
		{
			...
			{
				FComputeShaderRHIRef Shader = RHICreateComputeShader(Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
				if (IsValidRef(Shader))
				{

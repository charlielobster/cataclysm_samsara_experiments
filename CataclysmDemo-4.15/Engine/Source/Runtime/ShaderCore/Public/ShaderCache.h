...
public:
	...
	FPixelShaderRHIRef GetPixelShader(EShaderPlatform Platform, FSHAHash Hash, TArray<uint8> const& Code, int NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
	...
	FComputeShaderRHIRef GetComputeShader(EShaderPlatform Platform, TArray<uint8> const& Code, int NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
	...
	static FORCEINLINE void LogShader(EShaderPlatform Platform, EShaderFrequency Frequency, FSHAHash Hash, TArray<uint8> const& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
	{
		if ( Cache )
		{
			Cache->InternalLogShader(Platform, Frequency, Hash, Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
		}
	}
...
private:
	...
	void InternalLogShader(EShaderPlatform Platform, EShaderFrequency Frequency, FSHAHash Hash, TArray<uint8> const& Code, int NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
	...
	void SubmitShader(FShaderCacheKey const& Key, TArray<uint8> const& Code, int NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders

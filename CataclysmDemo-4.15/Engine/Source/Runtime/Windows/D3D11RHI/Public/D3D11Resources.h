...
// BEGIN CATACLYSM Extend RHI for VTR
class FD3D11TilePool : public FRHITilePool, public FD3D11BaseShaderResource
{
public:

	TRefCountPtr<ID3D11Buffer> Resource;

	FD3D11TilePool(ID3D11Buffer* InResource, uint32 InTilesX, uint32 InTilesY, uint32 InTilesZ, uint32 InNumTiles, uint32 InSizePerTile)
	: FRHITilePool(InTilesX, InTilesY, InTilesZ, InNumTiles, InSizePerTile)
	, Resource(InResource)
	{}

	virtual ~FD3D11TilePool()
	{
		UpdateBufferStats(Resource, false);
	}
...
// BEGIN CATACLYSM Extend RHI for VTR
template<>
struct TD3D11ResourceTraits<FRHITilePool>
{
	typedef FD3D11TilePool TConcreteType;
};
// END CATACLYSM Extend RHI for VTR

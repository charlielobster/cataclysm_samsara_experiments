...
#include "nvapi.h" // CATACLYSM more atomic float ops in shaders
...
FPixelShaderRHIRef FD3D11DynamicRHI::RHICreatePixelShader(const TArray<uint8>& Code, int NvShaderExtnSlot)// CATACLYSM more atomic float ops in shaders
{
	FShaderCodeReader ShaderCode(Code);
	...
	// CATACLYSM Begin
	if (NvShaderExtnSlot >= 0)
	{
		NvAPI_Status Status = NvAPI_D3D11_SetNvShaderExtnSlot(Direct3DDevice, NvU32(NvShaderExtnSlot));
	...
	if (NvShaderExtnSlot >= 0)
	{
		NvAPI_D3D11_SetNvShaderExtnSlot(Direct3DDevice, ~0u);
	}
	// CATACLYSM End
...
FComputeShaderRHIRef FD3D11DynamicRHI::RHICreateComputeShader(const TArray<uint8>& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
{ 
	FShaderCodeReader ShaderCode(Code);

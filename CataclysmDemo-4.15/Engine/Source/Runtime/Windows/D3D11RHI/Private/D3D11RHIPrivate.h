...
public:
	...
	virtual FPixelShaderRHIRef RHICreatePixelShader(const TArray<uint8>& Code, int NvShaderExtnSlot) final override; // CATACLYSM more atomic float ops in shaders
	...
	virtual FComputeShaderRHIRef RHICreateComputeShader(const TArray<uint8>& Code, int NvShaderExtnSlot) final override; // CATACLYSM more atomic float ops in shaders
	...
	virtual void RHIUpdateStructuredBuffer(FStructuredBufferRHIParamRef StructuredBuffer, const FUpdateTextureRegion2D& UpdateRegion,uint32 SourcePitch,const uint8* SourceData) final override;
	virtual void RHICopyStructuredBuffer(FStructuredBufferRHIParamRef SourceBuffer, FStructuredBufferRHIParamRef DestBuffer) final override;
	virtual void RHICopyStructureCount(FUnorderedAccessViewRHIParamRef SourceUAV, FStructuredBufferRHIParamRef DestBuffer) final override;	
	virtual FTilePoolRHIRef RHICreateTilePool(FTextureRHIParamRef Texture, uint32 Size) final override;
	virtual void RHIResizeTilePool(FTilePoolRHIParamRef TilePool, uint32 Size) final override;
	virtual void RHIUpdateTileMappings(FTextureRHIParamRef Texture, uint32 NumTiledResourceRegions, const TArray<FTiledResourceCoordinate>& StartCoordinates, const TArray<FTileRegionSize>& Sizes, FTilePoolRHIParamRef TilePool, uint32 NumRanges, const TArray<uint32>& Flags, const TArray<uint32>& StartOffsets, const TArray<uint32>& Counts) final override;
	...
	virtual void RHISetRenderTargets(uint32 NumSimultaneousRenderTargets, const FRHIRenderTargetView* NewRenderTargets, const FRHIDepthRenderTargetView* NewDepthStencilTarget, uint32 NumUAVs, const FUnorderedAccessViewRHIParamRef* UAVs, int initUAVCount = -1) final override; // CATACLYSM more atomic float ops in shaders
...
private:
	HMODULE HBAODLLHandle;
	GFSDK_SSAO_Context_D3D11* HBAOContext;
	void CreateHbaoInterface();
	void ReleaseHbaoInterface();
public:
#endif
	// NVCHANGE_END: Add HBAO+
...
protected:
	...
	void CommitRenderTargetsAndUAVs(int initUAVCount = -1); // CATACLYSM more atomic float ops in shaders

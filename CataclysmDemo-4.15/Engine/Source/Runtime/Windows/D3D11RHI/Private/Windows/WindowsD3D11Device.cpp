...
static bool SafeTestD3D11CreateDevice(IDXGIAdapter* Adapter,D3D_FEATURE_LEVEL MaxFeatureLevel,D3D_FEATURE_LEVEL* OutFeatureLevel)
{
	...
	{
		D3D_FEATURE_LEVEL_11_1, // CATACLYSM Extend RHI for VTR
...
const TCHAR* GetFeatureLevelString(D3D_FEATURE_LEVEL FeatureLevel)
{
	switch(FeatureLevel)
	{
		...
		case D3D_FEATURE_LEVEL_11_1:	return TEXT("11_1"); // CATACLYSM Extend RHI for VTR
	}
	return TEXT("X_X");
}
...
void FD3D11DynamicRHI::InitD3DDevice()
{
	...
	if(!Direct3DDevice)
	{
		...
		HRESULT hr = Direct3DDevice->CheckFeatureSupport(D3D11_FEATURE_D3D11_OPTIONS2, &Features, sizeof(Features));
        if (SUCCEEDED(hr))
            GRHISupportsVTR = Features.TiledResourcesTier >= D3D11_TILED_RESOURCES_TIER_3;
        else
            GRHISupportsVTR = false;
		// END CATACLYSM Extend RHI for VTR
		...
		if(DeviceFlags & D3D11_CREATE_DEVICE_DEBUG)
		{
			...
			if (InfoQueue)
			{
				...
				D3D11_MESSAGE_ID DenyIds[]  = {
                    ...
                    // We know that many tiles are NULL in our VTRs.
                    (D3D11_MESSAGE_ID)3146129,
                    // CATACLYSM End
				};
		...
		// BEGIN CATACLYSM more atomic float ops in shaders
#if PLATFORM_DESKTOP
		if( IsRHIDeviceNVIDIA() )
		{
			NvAPI_Initialize();
...
#endif
		// END CATACLYSM more atomic float ops in shaders

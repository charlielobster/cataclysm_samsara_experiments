...
void FD3D11DynamicRHI::CommitRenderTargetsAndUAVs(int initUAVCount) // CATACLYSM Extend RHI
{
	...
	for(uint32 UAVIndex = 0;UAVIndex < NumUAVs;++UAVIndex)
	{
		...
		UAVInitialCountArray[UAVIndex] = initUAVCount; // CATACLYSM Extend RHI
	}
	...
	)
{
	...
	if (initUAVCount != -1)
	{
		CommitRenderTargetsAndUAVs(initUAVCount);
	}

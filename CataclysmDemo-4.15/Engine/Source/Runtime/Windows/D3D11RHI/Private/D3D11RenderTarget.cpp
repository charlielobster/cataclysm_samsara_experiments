...
void FD3D11DynamicRHI::RHICopyToResolveTarget(FTextureRHIParamRef SourceTextureRHI, FTextureRHIParamRef DestTextureRHI, bool bKeepOriginalSurface, const FResolveParams& ResolveParams)
{
	...
	if(SourceTexture2D && DestTexture2D)
	{
		...
		if(SourceTexture2D != DestTexture2D)
		{
			...
			if(FeatureLevel >= D3D_FEATURE_LEVEL_11_0 // BEGIN CATACLYSM Extend RHI for VTR
			...
			// BEGIN CATACLYSM
			//atatarinov_begin
			// Implement the upsampling as a part of ResolveTextureUsingShader
			else if (FeatureLevel >= D3D_FEATURE_LEVEL_11_0
				&& DestTexture2D->GetDepthStencilView(FExclusiveDepthStencil::DepthWrite_StencilWrite)
				&& DestTexture2D->GetSizeX() < SourceTexture2D->GetSizeX()
				&& DestTexture2D->GetSizeY() < SourceTexture2D->GetSizeY())
			{
				D3D11_TEXTURE2D_DESC ResolveTargetDesc;

				DestTexture2D->GetResource()->GetDesc(&ResolveTargetDesc);

				ResolveTextureUsingShader<FResolveDepthDownsampleNonMSPS>(
					RHICmdList,
					SourceTexture2D,
					DestTexture2D,
					DestTexture2D->GetRenderTargetView(0, -1),
					DestTexture2D->GetDepthStencilView(FExclusiveDepthStencil::DepthWrite_StencilWrite),
					ResolveTargetDesc,
					GetDefaultRect(ResolveParams.Rect, SourceTexture2D->GetSizeX(), SourceTexture2D->GetSizeY()),
					GetDefaultRect(ResolveParams.Rect, DestTexture2D->GetSizeX(), DestTexture2D->GetSizeY()),
					Direct3DDeviceIMContext,
					FDummyResolveParameter()
					);
				...
				&& DestTexture2D->GetSizeY() < SourceTexture2D->GetSizeY())
			{
				...
				ResolveTextureUsingShader<FResolveDownsampleNonMSPS>(
					RHICmdList,
					SourceTexture2D,
					DestTexture2D,
					DestTexture2D->GetRenderTargetView(0, -1),

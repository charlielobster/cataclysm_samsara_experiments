...
/** Encapsulates a GPU read structured buffer with its UAV and SRV. */
struct FReadBufferStructured
{
	...
	void UpdateBuffer(uint32 X, uint32 Y, uint32 Width, uint32 Height, uint32 SourcePitch, const uint8* SourceData)
	{
		...
		RHIUpdateStructuredBuffer(Buffer, UpdateRegion, SourcePitch, SourceData);
	}
};
...
// BEGIN CATACLYSM Extend RHI for VTR
// VTR
template <bool FAKE_VTR>
struct FVTRStructured
{
	FTexture3DRHIRef Texture3D;
	FUnorderedAccessViewRHIRef UAV;
	FTilePoolRHIRef Buffer;

	FVTRStructured() {}

	void Initialize(uint32 SizeX, uint32 SizeY, uint32 SizeZ, EPixelFormat Format, uint32 NumMips, uint32 NumTilePoolBytes, uint32 AdditionalUsage = 0)
	{

		if (FAKE_VTR)
		{
			FRHIResourceCreateInfo CreateInfo;
			Texture3D = RHICreateTexture3D(SizeX, SizeY, SizeZ, Format, NumMips, TexCreate_ShaderResource | TexCreate_UAV | AdditionalUsage, CreateInfo);
			UAV = RHICreateUnorderedAccessView(Texture3D);
		}
		else
		{
			check(GRHISupportsVTR);

			FRHIResourceCreateInfo CreateInfo;
			Texture3D = RHICreateTexture3D(SizeX, SizeY, SizeZ, Format, NumMips, TexCreate_ShaderResource | TexCreate_UAV | TexCreate_TiledResource | AdditionalUsage, CreateInfo);
			UAV = RHICreateUnorderedAccessView(Texture3D);
			Buffer = RHICreateTilePool(Texture3D, NumTilePoolBytes);
		}
	}

	void Release()
	{
		Texture3D.SafeRelease();
		UAV.SafeRelease();
		if (!FAKE_VTR) Buffer.SafeRelease();
	}

	void ResizeTilePool(uint32 NumTiles)
	{
		if (!FAKE_VTR) RHIResizeTilePool(Buffer, NumTiles * Buffer->GetSizePerTile());
	}

	void UpdateTileMappings(uint32 NumTiledResourceRegions, const TArray<FTiledResourceCoordinate>& StartCoordinates, const TArray<FTileRegionSize>& Sizes, uint32 NumRanges, const TArray<uint32>& Flags, const TArray<uint32>& StartOffsets, const TArray<uint32>& Counts)
	{
		if (!FAKE_VTR) RHIUpdateTileMappings(Texture3D, NumTiledResourceRegions, StartCoordinates, Sizes, Buffer, NumRanges, Flags, StartOffsets, Counts);
	}
};
// BEGIN CATACLYSM Extend RHI for VTR
	...
	)
{
	...
	RHICmdList.SetRenderTargets(NewNumSimultaneousRenderTargets, RTVs, &DepthRTV, NewNumUAVs, UAVs, initUAVCount); // CATACLYSM Extend RHI
}

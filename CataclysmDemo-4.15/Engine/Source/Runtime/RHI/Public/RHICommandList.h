...
struct FRHICommandSetRenderTargets : public FRHICommand<FRHICommandSetRenderTargets>
{
	...
	int InitUAVCount; // CATACLYSM Extend RHI
		...
		)
	{
		...
		if (Bypass())
		{
			CMD_CONTEXT(RHISetRenderTargets)(
				...
				NewDepthStencilTargetRHI,
				NewNumUAVs,
				UAVs, initUAVCount); // CATACLYSM Extend RHI
			return;
		}
...
public:
	...
	FORCEINLINE FPixelShaderRHIRef CreatePixelShader(const TArray<uint8>& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
	{
		FScopedRHIThreadStaller StallRHIThread(*this);
		return GDynamicRHI->RHICreatePixelShader(Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
	}
	...
	FORCEINLINE FComputeShaderRHIRef CreateComputeShader(const TArray<uint8>& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
	{
		FScopedRHIThreadStaller StallRHIThread(*this);
		return GDynamicRHI->RHICreateComputeShader(Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
	}
	...
	// BEGIN CATACLYSM Extend RHI for VTR
	FORCEINLINE void UpdateStructuredBuffer(FStructuredBufferRHIParamRef StructuredBuffer, const FUpdateTextureRegion2D& UpdateRegion,uint32 SourcePitch,const uint8* SourceData)
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_RHIMETHOD_UpdateStructuredBuffer_Flush);
		ImmediateFlush(EImmediateFlushType::FlushRHIThread); 
		  
		GDynamicRHI->RHIUpdateStructuredBuffer(StructuredBuffer, UpdateRegion, SourcePitch, SourceData);
	}

	FORCEINLINE void CopyStructuredBuffer(FStructuredBufferRHIParamRef SourceBuffer, FStructuredBufferRHIParamRef DestBuffer)
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_RHIMETHOD_CopyStructuredBuffer_Flush);
		ImmediateFlush(EImmediateFlushType::FlushRHIThread); 
		GDynamicRHI->RHICopyStructuredBuffer(SourceBuffer, DestBuffer);
	}

	FORCEINLINE void CopyStructureCount(FUnorderedAccessViewRHIParamRef SourceUAV, FStructuredBufferRHIParamRef DestBuffer)
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_RHIMETHOD_CopyStructureCount_Flush);
		ImmediateFlush(EImmediateFlushType::FlushRHIThread); 
		GDynamicRHI->RHICopyStructureCount(SourceUAV, DestBuffer);
	}

	FORCEINLINE FTilePoolRHIRef CreateTilePool(FTextureRHIParamRef Texture, uint32 Size)
	{
		FScopedRHIThreadStaller StallRHIThread(*this);
		return GDynamicRHI->RHICreateTilePool(Texture, Size);
	}

	FORCEINLINE void ResizeTilePool(FTilePoolRHIParamRef TilePool, uint32 Size)
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_RHIMETHOD_ResizeTilePool_Flush);
		ImmediateFlush(EImmediateFlushType::FlushRHIThread); 
		GDynamicRHI->RHIResizeTilePool(TilePool, Size);
	}

	FORCEINLINE void UpdateTileMappings(FTextureRHIParamRef Texture, uint32 NumTiledResourceRegions, const TArray<FTiledResourceCoordinate>& StartCoordinates, const TArray<FTileRegionSize>& Sizes,
											 FTilePoolRHIParamRef TilePool, uint32 NumRanges, const TArray<uint32>& Flags, const TArray<uint32>& StartOffsets, const TArray<uint32>& Counts)
	{
		QUICK_SCOPE_CYCLE_COUNTER(STAT_RHIMETHOD_UpdateTileMappings_Flush);
		ImmediateFlush(EImmediateFlushType::FlushRHIThread); 
		GDynamicRHI->RHIUpdateTileMappings(Texture, NumTiledResourceRegions, StartCoordinates, Sizes, TilePool, NumRanges, Flags, StartOffsets, Counts);
	}
	// END CATACLYSM Extend RHI for VTR
...
FORCEINLINE FPixelShaderRHIRef RHICreatePixelShader(const TArray<uint8>& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
{
	return FRHICommandListExecutor::GetImmediateCommandList().CreatePixelShader(Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
}
...
FORCEINLINE FComputeShaderRHIRef RHICreateComputeShader(const TArray<uint8>& Code, int NvShaderExtnSlot) // CATACLYSM more atomic float ops in shaders
{
	return FRHICommandListExecutor::GetImmediateCommandList().CreateComputeShader(Code, NvShaderExtnSlot); // CATACLYSM more atomic float ops in shaders
}
...
// BEGIN CATACLYSM Extend RHI for VTR
FORCEINLINE void RHIUpdateStructuredBuffer(FStructuredBufferRHIParamRef StructuredBuffer, const FUpdateTextureRegion2D& UpdateRegion,uint32 SourcePitch,const uint8* SourceData)
{
	 FRHICommandListExecutor::GetImmediateCommandList().UpdateStructuredBuffer(StructuredBuffer, UpdateRegion, SourcePitch, SourceData);
}

FORCEINLINE void RHICopyStructuredBuffer(FStructuredBufferRHIParamRef SourceBuffer, FStructuredBufferRHIParamRef DestBuffer)
{
	 FRHICommandListExecutor::GetImmediateCommandList().CopyStructuredBuffer(SourceBuffer, DestBuffer);
}

FORCEINLINE void RHICopyStructureCount(FUnorderedAccessViewRHIParamRef SourceUAV, FStructuredBufferRHIParamRef DestBuffer)
{
	 FRHICommandListExecutor::GetImmediateCommandList().CopyStructureCount(SourceUAV, DestBuffer);
}

FORCEINLINE FTilePoolRHIRef RHICreateTilePool(FTextureRHIParamRef Texture, uint32 Size)
{
	return FRHICommandListExecutor::GetImmediateCommandList().CreateTilePool(Texture, Size);
}

FORCEINLINE void RHIResizeTilePool(FTilePoolRHIParamRef TilePool, uint32 Size)
{
	return FRHICommandListExecutor::GetImmediateCommandList().ResizeTilePool(TilePool, Size);
}

FORCEINLINE void RHIUpdateTileMappings(FTextureRHIParamRef Texture, uint32 NumTiledResourceRegions, const TArray<FTiledResourceCoordinate>& StartCoordinates, const TArray<FTileRegionSize>& Sizes,
									   FTilePoolRHIParamRef TilePool, uint32 NumRanges, const TArray<uint32>& Flags, const TArray<uint32>& StartOffsets, const TArray<uint32>& Counts)
{
	return FRHICommandListExecutor::GetImmediateCommandList().UpdateTileMappings(Texture, NumTiledResourceRegions, StartCoordinates, Sizes, TilePool, NumRanges, Flags, StartOffsets, Counts);
}
// END CATACLYSM Extend RHI for VTR

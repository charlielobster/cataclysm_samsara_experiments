...
class FRHITilePool : public FRHIResource
{
public:

	/** Initialization constructor. */
	FRHITilePool(uint32 InTilesX, uint32 InTilesY, uint32 InTilesZ, uint32 InNumTiles, uint32 InSizePerTile)
		: TilesX(InTilesX)
		, TilesY(InTilesY)
		, TilesZ(InTilesZ)
		, NumTiles(InNumTiles)
		, SizePerTile(InSizePerTile)
	{}

	uint32 GetMaxNumTiles() const { return TilesX * TilesY * TilesZ; }

	void UpdateNumTiles(uint32 InNumTiles) { NumTiles = InNumTiles; }

	uint32 GetNumTiles() const { return NumTiles; }

	uint32 GetSizePerTile() const { return SizePerTile; }
...
// BEGIN CATACLYSM Extend RHI for VTR
typedef FRHITilePool*              FTilePoolRHIParamRef;
typedef TRefCountPtr<FRHITilePool> FTilePoolRHIRef;
// END CATACLYSM Extend RHI for VTR

...
// This matches D3D12_TILE_RANGE_FLAG and, except for TRF_None, D3D11_TILE_RANGE_FLAG
enum ETileRangeFlag
{
	TRF_None = 0x00,
	TRF_NULL = 0x01,
...
// BEGIN CATACLYSM Extend RHI for VTR
// This matches D3D11_TILED_RESOURCE_COORDINATE and D3D11_TILED_RESOURCE_COORDINATE
struct FTiledResourceCoordinate
{
	uint32 X;
	uint32 Y;
	uint32 Z;
	...
	FTiledResourceCoordinate(uint32 InX, uint32 InY, uint32 InZ, uint32 InSubresource)
	: X(InX)
	, Y(InY)
	, Z(InZ)
...
// This matches D3D11_TILE_REGION_SIZE and D3D12_TILE_REGION_SIZE
struct FTileRegionSize
{
	uint32 NumTiles;
	int32 bUseBox; // BOOL is typedef int BOOL
	uint32 Width;
	uint16 Height;
	uint16 Depth;
	...
	FTileRegionSize(uint32 InNumTiles, int32 InUseBox, uint32 InWidth, uint16 InHeight, uint16 InDepth)
	: NumTiles(InNumTiles)
	, bUseBox(InUseBox)
	, Width(InWidth)
	, Height(InHeight)

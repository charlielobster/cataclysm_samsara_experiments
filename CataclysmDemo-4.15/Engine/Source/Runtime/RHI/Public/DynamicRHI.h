...
public:
	...
	*/
	...
	virtual void RHISetRenderTargets(uint32 NumSimultaneousRenderTargets, const FRHIRenderTargetView* NewRenderTargets, const FRHIDepthRenderTargetView* NewDepthStencilTarget, uint32 NumUAVs, const FUnorderedAccessViewRHIParamRef* UAVs, int initUAVCount = -1) = 0; // CATACLYSM Extend RHI
...
public:
	...
	// FlushType: Wait RHI Thread
	virtual FPixelShaderRHIRef RHICreatePixelShader(const TArray<uint8>& Code, int NvShaderExtnSlot = -1) = 0; // CATACLYSM more atomic float ops in shaders
	...
	// FlushType: Wait RHI Thread
	virtual FComputeShaderRHIRef RHICreateComputeShader(const TArray<uint8>& Code, int NvShaderExtnSlot = -1) = 0; // CATACLYSM more atomic float ops in shaders
	...
	*/
	...
	// CATACLYSM Begin
	virtual void RHIUpdateStructuredBuffer(FStructuredBufferRHIParamRef StructuredBuffer, const FUpdateTextureRegion2D& UpdateRegion,uint32 SourcePitch,const uint8* SourceData) = 0;
	
	virtual void RHICopyStructuredBuffer(FStructuredBufferRHIParamRef SourceBuffer, FStructuredBufferRHIParamRef DestBuffer) = 0;

	virtual void RHICopyStructureCount(FUnorderedAccessViewRHIParamRef SourceUAV, FStructuredBufferRHIParamRef DestBuffer) = 0;

	// Create a tile pool of the given texture.
	virtual FTilePoolRHIRef RHICreateTilePool(FTextureRHIParamRef Texture, uint32 Size) = 0;

	// Resizes a tile pool.
	virtual void RHIResizeTilePool(FTilePoolRHIParamRef TilePool, uint32 Size) = 0;

	// Updates mappings of tile locations in tiled resources to memory locations in a tile pool.
	virtual void RHIUpdateTileMappings(FTextureRHIParamRef Texture, uint32 NumTiledResourceRegions, const TArray<FTiledResourceCoordinate>& StartCoordinates, const TArray<FTileRegionSize>& Sizes, FTilePoolRHIParamRef TilePool, uint32 NumRanges, const TArray<uint32>& Flags, const TArray<uint32>& StartOffsets, const TArray<uint32>& Counts) = 0;
	// CATACLYSM End

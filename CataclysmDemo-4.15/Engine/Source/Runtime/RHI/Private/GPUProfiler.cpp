...
static void DumpStatsEventNode(FGPUProfilerEventNode* Node, float RootResult, int32 Depth, const FWildcardString& WildcardFilter, bool bParentMatchedFilter, FGPUProfileStatSummary& Summary,TMap<FString, float>* FluidTimes)// CATACLYSM
{
	...
	if (Node->NumDraws > 0 || Node->Children.Num() > 0 || Summary.bDumpEventLeafNodes)
	{
		...
		if (bMatchesFilter)
		{
			...
			if (FluidTimes)
			{
				float *shaderTime = FluidTimes->Find(Node->Name);
				if (shaderTime) *shaderTime += Node->TimingResult;
			}
			else
			{
				FString Extra;
		...
		for (int32 ChildIndex = 0; ChildIndex < Node->Children.Num(); ChildIndex++)
		{
			...
			DumpStatsEventNode(Node->Children[ChildIndex], RootResult, Depth + 1, WildcardFilter, bMatchesFilter, Summary, FluidTimes); // CATACLYSM
...
// CATACLYSM Begin
#define FluidTimeName_NUM 13
FString FluidTimeNames[FluidTimeName_NUM] =
{ "FluidSimulation"
, "FluidPreSim"
//, "FluidMapTiledResources"
//, "FluidZeroUnmappedBricks"
//, "FluidZeroActiveBricks"
, "FluidParticleVelocityUpdate"
//, "FluidSortParticles"
, "FluidGenerateTexcoords"
, "FluidVelocityToGrid"
, "FluidDensityToGrid"
, "FluidNewSurface"
//, "FluidNormalizeVelocityAndSplatFields"
//, "RedistanceLiquidBoundary"
//, "FluidExtendVelocity"
//, "FluidPrepLevelsetSplat"
, "FluidLevelsetSplat"
//, "RedistanceLiquidSurface"
//, "FluidConstrainVelocityWithSolidBoundary"
//, "FluidExtendLiquidIntoSolid"
//, "FluidDivergence"
, "FluidPressure"
, "FluidParticlePositionUpdate"
, "FluidDiffuseParticles"
, "FluidPostSimulate"
, "FluidRendering"
};
void FGPUProfilerEventNodeFrame::DumpEventTree()
{
	bool fluid_profile_only = false;
	if (EventTree.Num() > 0)
	{
		...
		static IConsoleVariable* CVRunSimProfiler = IConsoleManager::Get().FindConsoleVariable(TEXT("Fluid.RunSimProfiler"));
		static int32 fluidTimeFrame = 0;
		if (CVRunSimProfiler->GetInt() == 0)
		{
			fluidTimeFrame = 0;
		}
		else if (CVShowUI->GetInt() == 0 && CVScreenshot->GetInt() == 0)
		{
			fluid_profile_only = true;
		}

		if (!fluid_profile_only) UE_LOG(LogRHI, Warning, TEXT("Perf marker hierarchy, total GPU time %.2fms"), RootResult * 1000.0f);
		...
		if (!fluid_profile_only) LogDisjointQuery();
		...
		// CATACLYSM Really ugly stuff for debug timer on fluid simulations.
		static TMap<FString, float> *FluidTimes = nullptr;
		if (fluid_profile_only)
		{
			if (!FluidTimes)
			{
				FluidTimes = new TMap<FString, float>();

				for (int32 i = 0; i < FluidTimeName_NUM; ++i)
				{
					FluidTimes->Add(FluidTimeNames[i], 0);
				}
			}
			else
			{
				for (int32 i = 0; i < FluidTimeName_NUM; ++i)
				{
					(*FluidTimes)[FluidTimeNames[i]] = 0;
				}
			}
			if (!fluidTimeFrame)
			{
				UE_LOG(LogRHI, Warning, TEXT("Frame, %s,  %s, %s,  %s, %s,  %s, %s,  %s, %s,  %s, %s,  %s, %s"),//,  %s, %s,  %s, %s,  %s, %s,  %s, %s,  %s, %s,  %s, %s,  %s, %s,  %s, %s,  %s, %s,  %s"),
					*FluidTimeNames[0],
					*FluidTimeNames[1],
					*FluidTimeNames[2],
					*FluidTimeNames[3],
		...
		for (int32 BaseNodeIndex = 0; BaseNodeIndex < EventTree.Num(); BaseNodeIndex++)
		{
			DumpStatsEventNode(EventTree[BaseNodeIndex], RootResult, 0, RootWildcard, false, /*inout*/ Summary, FluidTimes);
		}
		if (fluid_profile_only)
		{
			UE_LOG(LogRHI, Warning, TEXT("%d, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f"),//,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f, %5.2f,  %5.2f"),
				fluidTimeFrame,
				(*FluidTimes)[FluidTimeNames[0]],
				(*FluidTimes)[FluidTimeNames[1]],
				(*FluidTimes)[FluidTimeNames[2]],
				(*FluidTimes)[FluidTimeNames[3]],
				(*FluidTimes)[FluidTimeNames[4]],
				(*FluidTimes)[FluidTimeNames[5]],
				(*FluidTimes)[FluidTimeNames[6]],
				(*FluidTimes)[FluidTimeNames[7]],
				(*FluidTimes)[FluidTimeNames[8]],
				(*FluidTimes)[FluidTimeNames[9]],
				(*FluidTimes)[FluidTimeNames[10]],
				(*FluidTimes)[FluidTimeNames[11]],
				(*FluidTimes)[FluidTimeNames[12]]/*,
				(*FluidTimes)[FluidTimeNames[13]],
				(*FluidTimes)[FluidTimeNames[14]],
				(*FluidTimes)[FluidTimeNames[15]],
				(*FluidTimes)[FluidTimeNames[16]],
				(*FluidTimes)[FluidTimeNames[17]],
				(*FluidTimes)[FluidTimeNames[18]],
				(*FluidTimes)[FluidTimeNames[19]],
				(*FluidTimes)[FluidTimeNames[20]],
				(*FluidTimes)[FluidTimeNames[21]],
				(*FluidTimes)[FluidTimeNames[22]],
				(*FluidTimes)[FluidTimeNames[23]],
				(*FluidTimes)[FluidTimeNames[24]],
				(*FluidTimes)[FluidTimeNames[25]],
				(*FluidTimes)[FluidTimeNames[26]],
				(*FluidTimes)[FluidTimeNames[27]],
				(*FluidTimes)[FluidTimeNames[28]],
				(*FluidTimes)[FluidTimeNames[29]],
				(*FluidTimes)[FluidTimeNames[30]],
				(*FluidTimes)[FluidTimeNames[31]]*/
				);
				
			UE_LOG(LogRHI, Warning, TEXT(""));
			UE_LOG(LogRHI, Warning, TEXT(""));
		}
		if (!fluid_profile_only) Summary.PrintSummary();
		// CATACLYSM End
